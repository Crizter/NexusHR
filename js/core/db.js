...
export const updateEmployeeAttendance = async (db, currentUserRole, userId, attendanceData) => { 
  ...
  return new Promise(async (resolve, reject) => { 
    try {
      ...
      const userStore = transaction.objectStore("users") ; // Fixed: was 'store', now 'userStore'
      
      // get the current user 
      const getUserRequest = userStore.get(userId) ; 
      
      getUserRequest.onsuccess = () => { // Fixed: was 'success', now 'onsuccess'
        ...
        // update the attendance part only
        const updatedUser = { 
          ...currentUser,
