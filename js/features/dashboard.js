...
const initializeDashboard = async () => {
  ...
  const [cacheErr] = tryCatchSync(() => loadStaleDataFromCache());
  if (cacheErr) console.warn("Cache load failed:", cacheErr);
  // Connect to database
  // db = await connectToDb();

  const [dbErr, dbConnection] = await tryCatchAsync(connectToDb());
  if (dbErr) {
    console.error(dbErr);
    return;
  }
  ...
  const [fetchErr] = await tryCatchAsync(fetchFreshDataAndUpdateCache());
  if (fetchErr) console.error("Fresh data fetch failed:", fetchErr);

  // Setup UI
  tryCatchSync(() => setupEventListeners());
  tryCatchSync(() => createModals());
  tryCatchSync(() => setGreeting());
...
const loadStaleDataFromCache = () => { 
    const [storageErr, cachedString] = tryCatchSync(() => localStorage.getItem(CACHE_KEY)) ; 
    if(storageErr) return showLoadingStates() ; 
    const [parseErr, parsedData] = tryCatchSync(()=> JSON.parse(cachedString)) ;
    if(parseErr){
        console.error("Cache corrupted");
        return showLoadingStates();
    }
...
const fetchFreshDataAndUpdateCache = async () => { 
    ...
    const [announcements, holidays, employee, allEmployees, teamActivity] = data; 
     const freshData = {
    ...
    tryCatchSync(() => renderAnnouncements(freshData.announcements));
    tryCatchSync(() => renderHolidays(freshData.holidays)) ; 
    tryCatchSync(() => updateAttendanceChart(freshData.attendance)) ; 
    tryCatchSync(() => renderTeamActivity(freshData.teamActivity)) ;     
    tryCatchSync(() => updateStatsCards(freshData, allEmployees)) ; 
    const cacheData = {
      data: freshData,
      timestamp: Date.now(),
    };
    tryCatchSync(()=> localStorage.setItem(CACHE_KEY, JSON.stringify(cacheData))) ;
...
const updateAttendanceChart = (attendanceData) => {
    const [error] = tryCatchSync(() => {
        ...
        // Defensive check inside the logic
...
const renderAnnouncements = (announcements) => {
    ...
    const [error] = tryCatchSync(() => {
      ...
      // RBAC MANAGEMENT 
        ...
        // Grab the user's preferred filter from local storage (e.g., 'Operations' or 'Tech')
        // Default to 'all' if they haven't set one yet
        const currentFilter = localStorage.getItem(FILTER_PREF_KEY) || "all";
        
        const filteredAnnouncements = filterAnnouncementsByBranch(
            announcements,
            currentFilter,
        );
...
const renderHolidays = (holidays) => {
    const [error] = tryCatchSync(() => {
        ...
        // Build the UI elements for the filtered list
...
/**
 * Generate mock team activity // for display purpose 
 */
...
const updateStatsCards = (data, allEmployees) => {
  ...
  if (holidaysCount) {
    const upcomingHolidays = data.holidays.filter(
...
const filterAnnouncementsByBranch = (announcements, branch) => {
  ...
  return announcements.filter((announcement) => {
    ...
    return (
      announcementBranch.toLowerCase() === branch.toLowerCase() ||
...
const createAnnouncementModal = () => {
  ...
  // Event listeners
  document
    .getElementById("close-announcement-modal")
    .addEventListener("click", hideAnnouncementModal);
  document
    .getElementById("cancel-announcement")
    .addEventListener("click", hideAnnouncementModal);
  document
...
const createHolidayModal = () => {
  ...
  // Event listeners
  document
    .getElementById("close-holiday-modal")
    .addEventListener("click", hideHolidayModal);
  document
    .getElementById("cancel-holiday")
    .addEventListener("click", hideHolidayModal);
  document
    .getElementById("holiday-form")
    .addEventListener("submit", handleHolidaySubmit);
...
const handleAnnouncementSubmit = async (e) => {
    ...
    // 2. Database Operation (Asynchronous)
    // We wrap the DB call in our utility. This allows us to handle the error 
    // right here without a nested try/catch block.
    const [addErr] = await tryCatchAsync(
        addAnnouncements(db, userRole, announcementData)
    );
    
    
    if (addErr) {
        console.error("[Dashboard] Failed to add announcement:", addErr);
        showNotification("Failed to post announcement. Please try again.", "error");
        return;
    }

    //   trigger a refresh of the dashboard data so the new post appears immediately.
    await fetchFreshDataAndUpdateCache();
